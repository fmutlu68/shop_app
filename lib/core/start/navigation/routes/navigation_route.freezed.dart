// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides

part of 'navigation_route.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more informations: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

/// @nodoc
class _$NavigationRouteTearOff {
  const _$NavigationRouteTearOff();

  NavigationRouteOnboard navigateToOnboard() {
    return const NavigationRouteOnboard();
  }

  NavigationRouteMainLogin navigateToMainLogin() {
    return const NavigationRouteMainLogin();
  }

  NavigationRouteHome navigateToHome() {
    return const NavigationRouteHome();
  }

  NavigationRouteLogin navigateToLogin() {
    return const NavigationRouteLogin();
  }

  NavigationRouteAccount navigateToAccount() {
    return const NavigationRouteAccount();
  }

  NavigationRouteFavorites navigateToFavorites() {
    return const NavigationRouteFavorites();
  }

  NavigationRouteShoppingCart navigateToShoppingCart() {
    return const NavigationRouteShoppingCart();
  }

  NavigationRouteFruitDetail navigateToFruitDetail(Fruit fruit) {
    return NavigationRouteFruitDetail(
      fruit,
    );
  }
}

/// @nodoc
const $NavigationRoute = _$NavigationRouteTearOff();

/// @nodoc
mixin _$NavigationRoute {
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) =>
      throw _privateConstructorUsedError;
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) =>
      throw _privateConstructorUsedError;
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) =>
      throw _privateConstructorUsedError;
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $NavigationRouteCopyWith<$Res> {
  factory $NavigationRouteCopyWith(
          NavigationRoute value, $Res Function(NavigationRoute) then) =
      _$NavigationRouteCopyWithImpl<$Res>;
}

/// @nodoc
class _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteCopyWith<$Res> {
  _$NavigationRouteCopyWithImpl(this._value, this._then);

  final NavigationRoute _value;
  // ignore: unused_field
  final $Res Function(NavigationRoute) _then;
}

/// @nodoc
abstract class $NavigationRouteOnboardCopyWith<$Res> {
  factory $NavigationRouteOnboardCopyWith(NavigationRouteOnboard value,
          $Res Function(NavigationRouteOnboard) then) =
      _$NavigationRouteOnboardCopyWithImpl<$Res>;
}

/// @nodoc
class _$NavigationRouteOnboardCopyWithImpl<$Res>
    extends _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteOnboardCopyWith<$Res> {
  _$NavigationRouteOnboardCopyWithImpl(NavigationRouteOnboard _value,
      $Res Function(NavigationRouteOnboard) _then)
      : super(_value, (v) => _then(v as NavigationRouteOnboard));

  @override
  NavigationRouteOnboard get _value => super._value as NavigationRouteOnboard;
}

/// @nodoc

class _$NavigationRouteOnboard implements NavigationRouteOnboard {
  const _$NavigationRouteOnboard();

  @override
  String toString() {
    return 'NavigationRoute.navigateToOnboard()';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) || (other is NavigationRouteOnboard);
  }

  @override
  int get hashCode => runtimeType.hashCode;

  @override
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) {
    return navigateToOnboard();
  }

  @override
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToOnboard != null) {
      return navigateToOnboard();
    }
    return orElse();
  }

  @override
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) {
    return navigateToOnboard(this);
  }

  @override
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToOnboard != null) {
      return navigateToOnboard(this);
    }
    return orElse();
  }
}

abstract class NavigationRouteOnboard implements NavigationRoute {
  const factory NavigationRouteOnboard() = _$NavigationRouteOnboard;
}

/// @nodoc
abstract class $NavigationRouteMainLoginCopyWith<$Res> {
  factory $NavigationRouteMainLoginCopyWith(NavigationRouteMainLogin value,
          $Res Function(NavigationRouteMainLogin) then) =
      _$NavigationRouteMainLoginCopyWithImpl<$Res>;
}

/// @nodoc
class _$NavigationRouteMainLoginCopyWithImpl<$Res>
    extends _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteMainLoginCopyWith<$Res> {
  _$NavigationRouteMainLoginCopyWithImpl(NavigationRouteMainLogin _value,
      $Res Function(NavigationRouteMainLogin) _then)
      : super(_value, (v) => _then(v as NavigationRouteMainLogin));

  @override
  NavigationRouteMainLogin get _value =>
      super._value as NavigationRouteMainLogin;
}

/// @nodoc

class _$NavigationRouteMainLogin implements NavigationRouteMainLogin {
  const _$NavigationRouteMainLogin();

  @override
  String toString() {
    return 'NavigationRoute.navigateToMainLogin()';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) || (other is NavigationRouteMainLogin);
  }

  @override
  int get hashCode => runtimeType.hashCode;

  @override
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) {
    return navigateToMainLogin();
  }

  @override
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToMainLogin != null) {
      return navigateToMainLogin();
    }
    return orElse();
  }

  @override
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) {
    return navigateToMainLogin(this);
  }

  @override
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToMainLogin != null) {
      return navigateToMainLogin(this);
    }
    return orElse();
  }
}

abstract class NavigationRouteMainLogin implements NavigationRoute {
  const factory NavigationRouteMainLogin() = _$NavigationRouteMainLogin;
}

/// @nodoc
abstract class $NavigationRouteHomeCopyWith<$Res> {
  factory $NavigationRouteHomeCopyWith(
          NavigationRouteHome value, $Res Function(NavigationRouteHome) then) =
      _$NavigationRouteHomeCopyWithImpl<$Res>;
}

/// @nodoc
class _$NavigationRouteHomeCopyWithImpl<$Res>
    extends _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteHomeCopyWith<$Res> {
  _$NavigationRouteHomeCopyWithImpl(
      NavigationRouteHome _value, $Res Function(NavigationRouteHome) _then)
      : super(_value, (v) => _then(v as NavigationRouteHome));

  @override
  NavigationRouteHome get _value => super._value as NavigationRouteHome;
}

/// @nodoc

class _$NavigationRouteHome implements NavigationRouteHome {
  const _$NavigationRouteHome();

  @override
  String toString() {
    return 'NavigationRoute.navigateToHome()';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) || (other is NavigationRouteHome);
  }

  @override
  int get hashCode => runtimeType.hashCode;

  @override
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) {
    return navigateToHome();
  }

  @override
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToHome != null) {
      return navigateToHome();
    }
    return orElse();
  }

  @override
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) {
    return navigateToHome(this);
  }

  @override
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToHome != null) {
      return navigateToHome(this);
    }
    return orElse();
  }
}

abstract class NavigationRouteHome implements NavigationRoute {
  const factory NavigationRouteHome() = _$NavigationRouteHome;
}

/// @nodoc
abstract class $NavigationRouteLoginCopyWith<$Res> {
  factory $NavigationRouteLoginCopyWith(NavigationRouteLogin value,
          $Res Function(NavigationRouteLogin) then) =
      _$NavigationRouteLoginCopyWithImpl<$Res>;
}

/// @nodoc
class _$NavigationRouteLoginCopyWithImpl<$Res>
    extends _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteLoginCopyWith<$Res> {
  _$NavigationRouteLoginCopyWithImpl(
      NavigationRouteLogin _value, $Res Function(NavigationRouteLogin) _then)
      : super(_value, (v) => _then(v as NavigationRouteLogin));

  @override
  NavigationRouteLogin get _value => super._value as NavigationRouteLogin;
}

/// @nodoc

class _$NavigationRouteLogin implements NavigationRouteLogin {
  const _$NavigationRouteLogin();

  @override
  String toString() {
    return 'NavigationRoute.navigateToLogin()';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) || (other is NavigationRouteLogin);
  }

  @override
  int get hashCode => runtimeType.hashCode;

  @override
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) {
    return navigateToLogin();
  }

  @override
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToLogin != null) {
      return navigateToLogin();
    }
    return orElse();
  }

  @override
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) {
    return navigateToLogin(this);
  }

  @override
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToLogin != null) {
      return navigateToLogin(this);
    }
    return orElse();
  }
}

abstract class NavigationRouteLogin implements NavigationRoute {
  const factory NavigationRouteLogin() = _$NavigationRouteLogin;
}

/// @nodoc
abstract class $NavigationRouteAccountCopyWith<$Res> {
  factory $NavigationRouteAccountCopyWith(NavigationRouteAccount value,
          $Res Function(NavigationRouteAccount) then) =
      _$NavigationRouteAccountCopyWithImpl<$Res>;
}

/// @nodoc
class _$NavigationRouteAccountCopyWithImpl<$Res>
    extends _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteAccountCopyWith<$Res> {
  _$NavigationRouteAccountCopyWithImpl(NavigationRouteAccount _value,
      $Res Function(NavigationRouteAccount) _then)
      : super(_value, (v) => _then(v as NavigationRouteAccount));

  @override
  NavigationRouteAccount get _value => super._value as NavigationRouteAccount;
}

/// @nodoc

class _$NavigationRouteAccount implements NavigationRouteAccount {
  const _$NavigationRouteAccount();

  @override
  String toString() {
    return 'NavigationRoute.navigateToAccount()';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) || (other is NavigationRouteAccount);
  }

  @override
  int get hashCode => runtimeType.hashCode;

  @override
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) {
    return navigateToAccount();
  }

  @override
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToAccount != null) {
      return navigateToAccount();
    }
    return orElse();
  }

  @override
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) {
    return navigateToAccount(this);
  }

  @override
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToAccount != null) {
      return navigateToAccount(this);
    }
    return orElse();
  }
}

abstract class NavigationRouteAccount implements NavigationRoute {
  const factory NavigationRouteAccount() = _$NavigationRouteAccount;
}

/// @nodoc
abstract class $NavigationRouteFavoritesCopyWith<$Res> {
  factory $NavigationRouteFavoritesCopyWith(NavigationRouteFavorites value,
          $Res Function(NavigationRouteFavorites) then) =
      _$NavigationRouteFavoritesCopyWithImpl<$Res>;
}

/// @nodoc
class _$NavigationRouteFavoritesCopyWithImpl<$Res>
    extends _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteFavoritesCopyWith<$Res> {
  _$NavigationRouteFavoritesCopyWithImpl(NavigationRouteFavorites _value,
      $Res Function(NavigationRouteFavorites) _then)
      : super(_value, (v) => _then(v as NavigationRouteFavorites));

  @override
  NavigationRouteFavorites get _value =>
      super._value as NavigationRouteFavorites;
}

/// @nodoc

class _$NavigationRouteFavorites implements NavigationRouteFavorites {
  const _$NavigationRouteFavorites();

  @override
  String toString() {
    return 'NavigationRoute.navigateToFavorites()';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) || (other is NavigationRouteFavorites);
  }

  @override
  int get hashCode => runtimeType.hashCode;

  @override
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) {
    return navigateToFavorites();
  }

  @override
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToFavorites != null) {
      return navigateToFavorites();
    }
    return orElse();
  }

  @override
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) {
    return navigateToFavorites(this);
  }

  @override
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToFavorites != null) {
      return navigateToFavorites(this);
    }
    return orElse();
  }
}

abstract class NavigationRouteFavorites implements NavigationRoute {
  const factory NavigationRouteFavorites() = _$NavigationRouteFavorites;
}

/// @nodoc
abstract class $NavigationRouteShoppingCartCopyWith<$Res> {
  factory $NavigationRouteShoppingCartCopyWith(
          NavigationRouteShoppingCart value,
          $Res Function(NavigationRouteShoppingCart) then) =
      _$NavigationRouteShoppingCartCopyWithImpl<$Res>;
}

/// @nodoc
class _$NavigationRouteShoppingCartCopyWithImpl<$Res>
    extends _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteShoppingCartCopyWith<$Res> {
  _$NavigationRouteShoppingCartCopyWithImpl(NavigationRouteShoppingCart _value,
      $Res Function(NavigationRouteShoppingCart) _then)
      : super(_value, (v) => _then(v as NavigationRouteShoppingCart));

  @override
  NavigationRouteShoppingCart get _value =>
      super._value as NavigationRouteShoppingCart;
}

/// @nodoc

class _$NavigationRouteShoppingCart implements NavigationRouteShoppingCart {
  const _$NavigationRouteShoppingCart();

  @override
  String toString() {
    return 'NavigationRoute.navigateToShoppingCart()';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) || (other is NavigationRouteShoppingCart);
  }

  @override
  int get hashCode => runtimeType.hashCode;

  @override
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) {
    return navigateToShoppingCart();
  }

  @override
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToShoppingCart != null) {
      return navigateToShoppingCart();
    }
    return orElse();
  }

  @override
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) {
    return navigateToShoppingCart(this);
  }

  @override
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToShoppingCart != null) {
      return navigateToShoppingCart(this);
    }
    return orElse();
  }
}

abstract class NavigationRouteShoppingCart implements NavigationRoute {
  const factory NavigationRouteShoppingCart() = _$NavigationRouteShoppingCart;
}

/// @nodoc
abstract class $NavigationRouteFruitDetailCopyWith<$Res> {
  factory $NavigationRouteFruitDetailCopyWith(NavigationRouteFruitDetail value,
          $Res Function(NavigationRouteFruitDetail) then) =
      _$NavigationRouteFruitDetailCopyWithImpl<$Res>;
  $Res call({Fruit fruit});
}

/// @nodoc
class _$NavigationRouteFruitDetailCopyWithImpl<$Res>
    extends _$NavigationRouteCopyWithImpl<$Res>
    implements $NavigationRouteFruitDetailCopyWith<$Res> {
  _$NavigationRouteFruitDetailCopyWithImpl(NavigationRouteFruitDetail _value,
      $Res Function(NavigationRouteFruitDetail) _then)
      : super(_value, (v) => _then(v as NavigationRouteFruitDetail));

  @override
  NavigationRouteFruitDetail get _value =>
      super._value as NavigationRouteFruitDetail;

  @override
  $Res call({
    Object? fruit = freezed,
  }) {
    return _then(NavigationRouteFruitDetail(
      fruit == freezed
          ? _value.fruit
          : fruit // ignore: cast_nullable_to_non_nullable
              as Fruit,
    ));
  }
}

/// @nodoc

class _$NavigationRouteFruitDetail implements NavigationRouteFruitDetail {
  const _$NavigationRouteFruitDetail(this.fruit);

  @override
  final Fruit fruit;

  @override
  String toString() {
    return 'NavigationRoute.navigateToFruitDetail(fruit: $fruit)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other is NavigationRouteFruitDetail &&
            (identical(other.fruit, fruit) ||
                const DeepCollectionEquality().equals(other.fruit, fruit)));
  }

  @override
  int get hashCode =>
      runtimeType.hashCode ^ const DeepCollectionEquality().hash(fruit);

  @JsonKey(ignore: true)
  @override
  $NavigationRouteFruitDetailCopyWith<NavigationRouteFruitDetail>
      get copyWith =>
          _$NavigationRouteFruitDetailCopyWithImpl<NavigationRouteFruitDetail>(
              this, _$identity);

  @override
  @optionalTypeArgs
  TResult when<TResult extends Object?>({
    required TResult Function() navigateToOnboard,
    required TResult Function() navigateToMainLogin,
    required TResult Function() navigateToHome,
    required TResult Function() navigateToLogin,
    required TResult Function() navigateToAccount,
    required TResult Function() navigateToFavorites,
    required TResult Function() navigateToShoppingCart,
    required TResult Function(Fruit fruit) navigateToFruitDetail,
  }) {
    return navigateToFruitDetail(fruit);
  }

  @override
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>({
    TResult Function()? navigateToOnboard,
    TResult Function()? navigateToMainLogin,
    TResult Function()? navigateToHome,
    TResult Function()? navigateToLogin,
    TResult Function()? navigateToAccount,
    TResult Function()? navigateToFavorites,
    TResult Function()? navigateToShoppingCart,
    TResult Function(Fruit fruit)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToFruitDetail != null) {
      return navigateToFruitDetail(fruit);
    }
    return orElse();
  }

  @override
  @optionalTypeArgs
  TResult map<TResult extends Object?>({
    required TResult Function(NavigationRouteOnboard value) navigateToOnboard,
    required TResult Function(NavigationRouteMainLogin value)
        navigateToMainLogin,
    required TResult Function(NavigationRouteHome value) navigateToHome,
    required TResult Function(NavigationRouteLogin value) navigateToLogin,
    required TResult Function(NavigationRouteAccount value) navigateToAccount,
    required TResult Function(NavigationRouteFavorites value)
        navigateToFavorites,
    required TResult Function(NavigationRouteShoppingCart value)
        navigateToShoppingCart,
    required TResult Function(NavigationRouteFruitDetail value)
        navigateToFruitDetail,
  }) {
    return navigateToFruitDetail(this);
  }

  @override
  @optionalTypeArgs
  TResult maybeMap<TResult extends Object?>({
    TResult Function(NavigationRouteOnboard value)? navigateToOnboard,
    TResult Function(NavigationRouteMainLogin value)? navigateToMainLogin,
    TResult Function(NavigationRouteHome value)? navigateToHome,
    TResult Function(NavigationRouteLogin value)? navigateToLogin,
    TResult Function(NavigationRouteAccount value)? navigateToAccount,
    TResult Function(NavigationRouteFavorites value)? navigateToFavorites,
    TResult Function(NavigationRouteShoppingCart value)? navigateToShoppingCart,
    TResult Function(NavigationRouteFruitDetail value)? navigateToFruitDetail,
    required TResult orElse(),
  }) {
    if (navigateToFruitDetail != null) {
      return navigateToFruitDetail(this);
    }
    return orElse();
  }
}

abstract class NavigationRouteFruitDetail implements NavigationRoute {
  const factory NavigationRouteFruitDetail(Fruit fruit) =
      _$NavigationRouteFruitDetail;

  Fruit get fruit => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $NavigationRouteFruitDetailCopyWith<NavigationRouteFruitDetail>
      get copyWith => throw _privateConstructorUsedError;
}
